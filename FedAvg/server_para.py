...
from client_para import ClientGroup, SingleClient
from multiprocessing import Pool
from copy import deepcopy
from tqdm import tqdm

def update_single_client(client, epoch, batch_size, model, loss_fct, optimizer, global_parameters):
    return client.local_update(epoch, batch_size, model, loss_fct, optimizer, global_parameters)
...
if __name__=="__main__":
    ...
    parser.add_argument('-cpu', '--cpu_for_para', type=int, default=1, help='Number of CPU for multiprocessing')
    ...
    n_workers = max(os.cpu_count(), args['cpu_for_para']) - 1
    assert n_workers > 0
    ...
    for i in tqdm(range(args['num_comm'])):
        ...
        with Pool(processes=n_workers) as pool:
            ...
            for client in tqdm(clients_in_comm):
                process = pool.apply_async(myClients.clients_set[client].local_update, (args['epoch'], args['batchsize'], deepcopy(model),
